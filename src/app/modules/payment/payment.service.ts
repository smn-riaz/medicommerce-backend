...
const makePaymentWithPrescription = async(data:any, paymentInfo:any) => {
    ...
    const sslcz = new SSLCommerz(config.ssl_store_id as string, config.ssl_store_password as string, config.ssl_is_live as boolean);
      ...
      try {
        ...
        const updatedOrder = await Order.findByIdAndUpdate(paymentInfo._id, {paymentStatus:true}, {
            new: true,
            runValidators: true,
          })
